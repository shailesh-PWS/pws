var Inventory = new function()
{
	...
	this.getInventoryJSON = function(obj){
        Global.LatestInventoryItemJSON = $.parseJSON(obj.DeltaJson);
		Global.InventoryItemJSON = $.parseJSON(obj.FullInventoryJson);

		Log.addAction("inventoryrecieved",$.parseJSON(obj.DeltaJson));
	}
	...
	this.addSecondaryOptionsMenuToTHIS = function(obj)
	{
		...
		$('.invBlockOptions .invBlockTabTabNormal').mousedown(function(){
	...
	this.buildTradeOptionsPanel = function(obj)
	{
		...
		//ADD LISTENERS
		$('.invBlockCompTabNormal[btn-id=saleupunit]').mousedown(function(){
		...
		$('.invBlockCompTabNormal[btn-id=saledownunit]').mousedown(function(){
		...
		$('.invBlockCompTabNormal[btn-id=qtyupunit]').mousedown(function(){
		...
		$('.invBlockCompTabNormal[btn-id=qtydownunit]').mousedown(function(){
		...
		$('.invBlockCompTabNormal[btn-id=confirm]').mousedown(function(){

			//alert("ITEM PUT ON SALE LIST")

		})
		$('.invBlockCompTabNormal[btn-id=cancel]').mousedown(function(){
